...
FROM 
 data.time_series
WHERE data.time_series.nuts_id= ?
),
...
FROM 
 data.euref_grid_points, data.nuts_XXXX
WHERE nuts_id = ? AND 
 data.euref_grid_points.geom && data.nuts_XXXX.geom AND
 ST_Intersects(data.euref_grid_points.geom, data.nuts_XXXX.geom)
),
